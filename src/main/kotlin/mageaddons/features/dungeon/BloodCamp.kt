...
import mageaddons.utils.RenderUtils.drawBox
import mageaddons.utils.RenderUtils.renderVec
import mageaddons.utils.clone
import mageaddons.utils.getSkullValue
import mageaddons.utils.toAABB
...
object BloodCamp: ModuleFactory(
    name = "Blood Camp helper",
    toggle = Config.bloodCampHelper
) {
...
//    }
    ...
    fun onPostMetadata(event: PostEntityMetadata){
        if (!inDungeons || inBoss || !toggle) return
        if (!toggle || currentWatcherEntity != null) return
    ...
    private fun getTime(firstSpawn: Boolean, timeTook: Long) = (if (firstSpawn) 2000 else 0) + (38 * 50) - timeTook + 40
